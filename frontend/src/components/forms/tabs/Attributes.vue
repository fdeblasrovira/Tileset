...
<template>
  <div class="border rounded-md border-tileset-grey-2 space-y-3 px-4 py-5 sm:p-6">
    ...
    <div v-if="attributes.length <= 0" class="w-full h-10 text-center items-center m-auto font-light italic">
      There are no attributes yet
    </div>
    <template v-for="(attribute, index) in attributes" :key="attribute.id">
      <div class="border rounded-md border-tileset-grey-2 space-y-3 px-4 py-5 sm:p-6">
        <Range v-model="attribute.defaultValue" :editable="true" :min="attribute.min" :max="attribute.max"
          :defaultValue="attribute.defaultValue" :leftColor="attribute.leftColor" :leftLabel="attribute.leftLabel"
          :rightColor="attribute.rightColor" :rightLabel="attribute.rightLabel">
        </Range>
        <div class="flex justify-between">
          <FullButton @click="displayAttributeDeleteModal(true, index)" text="Delete" color="bg-tileset-red"
            hover="hover:bg-tileset-red-1" class="mx-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
              stroke="currentColor" class="w-6 h-6 stroke-tileset-white">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
            </svg>
          </FullButton>
          <FullButton @click="displayAttributeEditModal(true, index)" text="Edit" color="bg-tileset-blue"
            hover="hover:bg-tileset-blue-1" class="mx-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
              stroke="currentColor" class="w-6 h-6 stroke-tileset-white">
              <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <FullButton @click="addAttribute" text="Add" color="bg-tileset-blue" hover="hover:bg-tileset-blue-1">
  ...
  <Modal :open="showAttributeDeleteModal" title="Delete attribute"
    ...
    <template #svg>
      <svg class="h-6 w-6 stroke-tileset-white -mt-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <template #buttons>
      <div class="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
        <button @click="deleteAttribute()" type="button"
        ...
        <button @click="showAttributeDeleteModal = false" type="button"
  ...
  <Modal :open="showAttributeEditModal" title="Edit attribute" description="Edit the values of the attribute"
    ...
    <template #contents>
      ...
      <div class="space-y-3 mt-3">
        <div class="flex">
          <Input v-model="editingAttribute.leftLabel" label="Left label" name="left-label" type="text" class="mr-2" />
          <Input v-model="editingAttribute.rightLabel" label="Right label" name="right-label" type="text"
            class="ml-2" />
        </div>
        <div class="flex">
          <Input v-model="editingAttribute.min" label="Minimum value" name="minimum-value" type="number" class="mr-2" />
          <Input v-model="editingAttribute.max" label="Maximum value" name="maximum-value" type="number" class="ml-2" />
        </div>
        <div class="flex">
          <ColorPicker v-model="editingAttribute.leftColor" label="Left Color" class="w-full h-10 mr-2"
            :display-color="editingAttribute.leftColor" />
          <ColorPicker v-model="editingAttribute.rightColor" label="Right Color" class="w-full h-10 ml-2"
            :display-color="editingAttribute.rightColor" />
        </div>
        <Input v-model="editingAttribute.defaultValue" label="Default value" name="default-value" type="number" />
      </div>
    </template>
    <template #buttons>
      <p v-if="attributeModalErrorMessage.length > 0"
        class="font-light text-tileset-red text-right text-sm italic px-4 sm:px-6">
        {{ attributeModalErrorMessage }}
      </p>
      <div class="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
        <button @click="editAttribute()" type="button"
        ...
        <button @click="showAttributeEditModal = false" type="button"
