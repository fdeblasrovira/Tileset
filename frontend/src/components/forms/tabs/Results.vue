...
<template>
  <div class="border rounded-md border-tileset-grey-2 space-y-3 px-4 py-5 sm:p-6">
    ...
    <div v-if="results.length <= 0" class="w-full h-10 text-center items-center m-auto font-light italic">
      There are no results yet
    </div>
    <ResultOption v-for="(result, index) in results" @delete-result="displayResultDeleteModal(true, index)"
      @edit-result="displayResultEditModal(true, index)" :data="result" :key="result.id" :index="index"
      :open="openedResult == result.id" :close="lastOpenedResult == result.id" />
    <FullButton @click="addResult()" text="Add" color="bg-tileset-blue" hover="hover:bg-tileset-blue-1">
  ...
  <Modal :open="showResultDeleteModal" title="Delete result" description="Are you sure you want to delete this result?"
    ...
    <template #buttons>
      <div class="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
        ...
        <button @click="showResultDeleteModal = false" type="button"
  ...
  <!-- Result edit modal -->
  <Modal :open="showResultEditModal" title="Edit result" description="Edit the result's parameters"
        ...
        <template v-for="(attribute) in attributes">
          <div class="border rounded-md border-tileset-grey-2 space-y-3 px-4 py-5 sm:p-6">
            <Range v-model="editingResult.attributeValues[attribute.id]" :editable="true" :min="attribute.min"
              ...
              :leftColor="attribute.leftColor" :leftLabel="attribute.leftLabel" :rightColor="attribute.rightColor"
    ...
    <template #buttons>
      ...
      <div class="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
        ...
        <button @click="showResultEditModal = false" type="button"
