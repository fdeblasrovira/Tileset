<script setup>
import { ref } from "vue";
const props = defineProps(["label", "image", "modelValue", "input"]);
  ...
  <div>
    ...
    <div class="my-3 flex items-center flex-col space-y-6">
      <span
        ...
        class="flex bg-cover h-48 min-w-[3rem] w-48 overflow-hidden rounded-3xl bg-tileset-grey-2 bg-contain bg-no-repeat bg-center border-4 border-tileset-grey-4"
      >
        <svg
          v-if="image.length <= 0"
