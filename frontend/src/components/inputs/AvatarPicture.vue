...
function previewFiles(event) {
  ...
  if (errorMessage.value.length <= 0) reader.readAsDataURL(selectedFile);
...
<template>
  <div>
    ...
    <div class="my-3 flex items-center flex-col space-y-6">
      ...
      <div v-else class="flex bg-cover h-24 min-w-[3rem] w-24 overflow-hidden rounded-full bg-tileset-grey-2 bg-contain bg-no-repeat bg-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        stroke="currentColor" class="flex w-12 h-12 m-auto self-center">
